Hooks.once("ready", () => {
  ...
  if (!DamageRoll) {
    console.warn("SWADE Weapon Properties | Could not find DamageRoll.");
    return;
  }
  ...
  async function resolveBrutalDie(term) {
    ...
    while (index < term.results.length) {
      ...
      if (safetyCounter > MAX_RESULTS) {
        console.error(
          "SWADE Weapon Properties | Safety limit reached while resolving a Brutal die.",
          term
        );
        break;
      }
...
Hooks.on(
  ...
  (actor, item, roll, modifiers, options) => {
    const hasBrutal =
      actor?.getFlag("swade-weapon-properties", "brutal") ||
      item?.getFlag("swade-weapon-properties", "brutal");
